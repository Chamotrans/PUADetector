...
The verification script runs `xcodegen generate`, Swift parse, privacy manifest lint, and the full Xcode test scheme. The test scheme runs both `PUADetectorTests` and `PUADetectorUITests`. UI tests pass `-UITestSkipSplash` so permission prompts do not block automated navigation checks.
